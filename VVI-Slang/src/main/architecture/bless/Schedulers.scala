...
object Schedulers {
  ...
  val threadNickNames: Map[String, Art.BridgeId] = Map(
    ISZ(
      Arch.PG_i_Instance_pp_t.name ~> Arch.PG_i_Instance_pp_t.id,
      Arch.PG_i_Instance_pp_rc.name ~> Arch.PG_i_Instance_pp_rc.id)
  )
  ...
  val PG_i_Instance_pp_t_timingProperties: ThreadTimingProperties = ThreadTimingProperties(
    computeExecutionTime = None(),
    domain = None())

  val PG_i_Instance_pp_rc_timingProperties: ThreadTimingProperties = ThreadTimingProperties(
    computeExecutionTime = None(),
    domain = None())
  ...
  val defaultDomainToBridgeIdMap: ISZ[Art.BridgeId] = ISZ(
    /* domain 0 */ Arch.PG_i_Instance_pp_t.id,
    /* domain 1 */ Arch.PG_i_Instance_pp_rc.id
  )
