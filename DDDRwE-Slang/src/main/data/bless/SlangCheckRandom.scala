...
@msig trait RandomLibI {
  ...
  def next_artDataContent(): art.DataContent = {
    ...
    var v: art.DataContent = c match {
      ...
      case _artDataContent_DataTypeId.MissingMISSING_AADL_TYPE_Payload_Id => (nextMissingMISSING_AADL_TYPE_Payload _).apply()
  ...
  // ============= Missing.MISSING_AADL_TYPE ===================

  def get_Config_MissingMISSING_AADL_TYPE: Config_MissingMISSING_AADL_TYPE
  def set_Config_MissingMISSING_AADL_TYPE(config: Config_MissingMISSING_AADL_TYPE): RandomLib

  def nextMissingMISSING_AADL_TYPE(): Missing.MISSING_AADL_TYPE = {

    var v: Missing.MISSING_AADL_TYPE = Missing.MISSING_AADL_TYPE()

    if(get_Config_MissingMISSING_AADL_TYPE.attempts >= 0) {
     for(i <- 0 to get_Config_MissingMISSING_AADL_TYPE.attempts) {
        if(get_Config_MissingMISSING_AADL_TYPE.filter(v)) {
          return v
        }
        if (get_Config_MissingMISSING_AADL_TYPE.verbose) {
          println(s"Retrying for failing value: $v")
        }
        v = Missing.MISSING_AADL_TYPE()
     }
    } else {
     while(T) {
       if(get_Config_MissingMISSING_AADL_TYPE.filter(v)) {
  ...
  // ============= Missing.MISSING_AADL_TYPE_Payload ===================

  def get_Config_MissingMISSING_AADL_TYPE_Payload: Config_MissingMISSING_AADL_TYPE_Payload
  def set_Config_MissingMISSING_AADL_TYPE_Payload(config: Config_MissingMISSING_AADL_TYPE_Payload): RandomLib

  def nextMissingMISSING_AADL_TYPE_Payload(): Missing.MISSING_AADL_TYPE_Payload = {
    var value: Missing.MISSING_AADL_TYPE = nextMissingMISSING_AADL_TYPE()

    var v: Missing.MISSING_AADL_TYPE_Payload = Missing.MISSING_AADL_TYPE_Payload(value)

    if(get_Config_MissingMISSING_AADL_TYPE_Payload.attempts >= 0) {
     for(i <- 0 to get_Config_MissingMISSING_AADL_TYPE_Payload.attempts) {
        if(get_Config_MissingMISSING_AADL_TYPE_Payload.filter(v)) {
          return v
        }
        if (get_Config_MissingMISSING_AADL_TYPE_Payload.verbose) {
          println(s"Retrying for failing value: $v")
        }
        value = nextMissingMISSING_AADL_TYPE()
        v = Missing.MISSING_AADL_TYPE_Payload(value)
     }
    } else {
     while(T) {
       if(get_Config_MissingMISSING_AADL_TYPE_Payload.filter(v)) {
...
@record class RandomLib(val gen: org.sireum.Random.Gen) extends RandomLibI {
  ...
  // ============= Missing.MISSING_AADL_TYPE ===================
  def alwaysTrue_MissingMISSING_AADL_TYPE(v: Missing.MISSING_AADL_TYPE): B = {return T}

  var config_MissingMISSING_AADL_TYPE: Config_MissingMISSING_AADL_TYPE = Config_MissingMISSING_AADL_TYPE(100, _verbose, alwaysTrue_MissingMISSING_AADL_TYPE _)

  def get_Config_MissingMISSING_AADL_TYPE: Config_MissingMISSING_AADL_TYPE = {return config_MissingMISSING_AADL_TYPE}

  def set_Config_MissingMISSING_AADL_TYPE(config: Config_MissingMISSING_AADL_TYPE): RandomLib ={
    config_MissingMISSING_AADL_TYPE = config
    return this
  }

  // ============= Missing.MISSING_AADL_TYPE_Payload ===================
  def alwaysTrue_MissingMISSING_AADL_TYPE_Payload(v: Missing.MISSING_AADL_TYPE_Payload): B = {return T}

  var config_MissingMISSING_AADL_TYPE_Payload: Config_MissingMISSING_AADL_TYPE_Payload = Config_MissingMISSING_AADL_TYPE_Payload(100, _verbose, alwaysTrue_MissingMISSING_AADL_TYPE_Payload _)

  def get_Config_MissingMISSING_AADL_TYPE_Payload: Config_MissingMISSING_AADL_TYPE_Payload = {return config_MissingMISSING_AADL_TYPE_Payload}

  def set_Config_MissingMISSING_AADL_TYPE_Payload(config: Config_MissingMISSING_AADL_TYPE_Payload): RandomLib ={
    config_MissingMISSING_AADL_TYPE_Payload = config
    return this
  }
}
