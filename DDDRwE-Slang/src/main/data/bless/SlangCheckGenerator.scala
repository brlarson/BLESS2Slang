...
@record class Gen_MissingMISSING_AADL_TYPE(param: RandomLibI) extends MJen[Missing.MISSING_AADL_TYPE] {
  override def generate(f: Missing.MISSING_AADL_TYPE => Jen.Action): Jen.Action = {
    var continue = Jen.Continue
    while (T) {

      continue = f(param.nextMissingMISSING_AADL_TYPE())
...
@record class Gen_MissingMISSING_AADL_TYPE_Payload(param: RandomLibI) extends MJen[Missing.MISSING_AADL_TYPE_Payload] {
  override def generate(f: Missing.MISSING_AADL_TYPE_Payload => Jen.Action): Jen.Action = {
    var continue = Jen.Continue
    while (T) {

      continue = f(param.nextMissingMISSING_AADL_TYPE_Payload())
