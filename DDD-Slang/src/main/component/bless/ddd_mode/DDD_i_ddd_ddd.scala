...
// generated by BLESS v3.2.1 at Wed Sep 27 13:12:02 CDT 2023 from ddd_mode::DDD.i
...
object DDD_i_ddd_ddd 
  ...
  var tops: F64 = f64"0.0"
  var last_vp_or_vs: F64 = f64"0.0"
  var last_ap_or_as: F64 = f64"0.0"
  var va_interval: F64 = (f64"1000.0" / 1000.0)-(f64"150.0" / 1000.0)
  var timeout_id : Art.PortId = Art.PortId.fromZ(Art.numPorts - 10)
  ...
  def Compute_Entrypoint(api: DDD_i_Operational_Api, Dispatch_Status: Set[art.Art.PortId]): B = 
    ...
    DDD_i_ddd_ddd_Bridge.timeout_as_Sensed_AV_Delay_ms_Duration = bless.util.BlessConversion.floatSecondToS64millisecond((f64"130.0" / 1000.0))
    DDD_i_ddd_ddd_Bridge.timeout_vp_vs_Lower_Rate_Limit_Interval_ms_Duration = bless.util.BlessConversion.floatSecondToS64millisecond((f64"1000.0" / 1000.0))
    DDD_i_ddd_ddd_Bridge.timeout_vp_vs_va_interval_Duration = bless.util.BlessConversion.floatSecondToS64millisecond(va_interval)
    return retval
  ...
  def executionState_check_atrial_refractories(api: DDD_i_Operational_Api): Unit = 
    ...
    (f64"200.0" / 1000.0)) > last_vp_or_vs)) & (((bless.util.BlessConversion.s64millisecondToFloatSecond(Art.time())-
    (f64"150.0" / 1000.0)) > last_ap_or_as)) )
  ...
  def executionState_check_vrp(api: DDD_i_Operational_Api): Unit = 
    {
    if ( ((bless.util.BlessConversion.s64millisecondToFloatSecond(Art.time())-
    (f64"300.0" / 1000.0)) < last_vp_or_vs) )
